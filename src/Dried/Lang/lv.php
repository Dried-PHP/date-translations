...
return [
    ...
    'calendar' => [
        ...
        'nextWeek' => static function (DateTimeInterface $current, DateTimeInterface $other) use ($daysOfWeekLocativum): string {
            if ($current->format('o-W') !== $other->format('o-W')) {
                return '[nākošo] ['.$daysOfWeekLocativum[(int) $current->format('w')].'] [plkst.] LT';
            }

            return '['.$daysOfWeekLocativum[(int) $current->format('w')].'] [plkst.] LT';
        },
        'lastDay' => '[vakar] [plkst.] LT',
        'lastWeek' => static function (DateTimeInterface $current) use ($daysOfWeekLocativum) {
            return '[pagājušo] ['.$daysOfWeekLocativum[(int) $current->format('w')].'] [plkst.] LT';
        },
        'sameElse' => 'L',
    ],
