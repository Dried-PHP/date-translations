...
$processHoursFunction = static function (DateTimeInterface $date, string $format) {
    return $format . 'о' . (((int) $date->format('G')) === 11 ? 'б' : '') . '] LT';
};
...
return [
    ...
    'calendar' => [
        'sameDay' => static fn (DateTimeInterface $date) => $processHoursFunction($date, '[Сьогодні '),
        'nextDay' => static fn (DateTimeInterface $date) => $processHoursFunction($date, '[Завтра '),
        'nextWeek' => static fn (DateTimeInterface $date) => $processHoursFunction($date, '[У] dddd ['),
        'lastDay' => static fn (DateTimeInterface $date) => $processHoursFunction($date, '[Вчора '),
    ...
    'weekdays' => static function (DateTimeInterface $date, $format, $index) {
        static $words = [
        ...
        $format ??= '';
        $nounCase = preg_match('/(\[([ВвУу])])\s+dddd/u', $format)
            ? 'accusative'
            : (
